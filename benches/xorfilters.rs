...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    let filter: BinaryFuse8 = bincode::decode_from_reader(reader, bincode::config::standard()).unwrap();
    c.bench_function("fuse8", |b| b.iter(|| fuse8(&filter, black_box(52))));
    drop(filter);

    let reader = BufReader::new(File::open("./data/xorfilter16").unwrap());
    let filter: BinaryFuse16 = bincode::decode_from_reader(reader, bincode::config::standard()).unwrap();
